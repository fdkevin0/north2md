...
type ValidationResult struct {
	IsValid       bool          `toml:"is_valid"`       // Cookie是否有效
	LoginStatus   LoginStatus   `toml:"login_status"`   // 登录状态
	TestURL       string        `toml:"test_url"`       // 测试URL
	TestedAt      time.Time     `toml:"tested_at"`      // 测试时间
	ResponseTime  time.Duration `toml:"response_time"`  // 响应时间
	ErrorMessage  string        `toml:"error_message"`  // 错误信息
	StatusCode    int           `toml:"status_code"`    // HTTP状态码
	ContentLength int64         `toml:"content_length"` // 内容长度
	RedirectURL   string        `toml:"redirect_url"`   // 重定向URL
	HasLoginWall  bool          `toml:"has_login_wall"` // 是否有登录墙
}

// ValidationOptions 验证配置
type ValidationOptions struct {
	TestTimeout    time.Duration `toml:"test_timeout"`    // 测试超时时间
	RetryCount     int           `toml:"retry_count"`     // 重试次数
	TestUserAgent  string        `toml:"test_user_agent"` // 测试用户代理
	EnableRedirect bool          `toml:"enable_redirect"` // 是否跟随重定向
	MaxRedirects   int           `toml:"max_redirects"`   // 最大重定向次数
}
